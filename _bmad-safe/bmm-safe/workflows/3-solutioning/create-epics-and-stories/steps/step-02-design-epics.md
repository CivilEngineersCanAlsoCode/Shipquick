...
# Path Definitions
workflow_path: "{project-root}/_bmad-safe/bmm-safe/workflows/3-solutioning/create-epics-and-stories"
...
# Task References
advancedElicitationTask: "{project-root}/_bmad-safe/core/workflows/advanced-elicitation/workflow.xml"
partyModeWorkflow: "{project-root}/_bmad-safe/core/workflows/party-mode/workflow.md"
...
### 2. Explain Epic Design Principles (SAFe-Aligned)

**SAFe HIERARCHY CONTEXT:**

In SAFe 6.0.1, work items follow this hierarchy:

- **Strategic Theme** → Business drivers from portfolio
- **Epic** → Large initiative spanning multiple PIs
- **Capability** (Large Solution) → Solution-level behavior
- **Feature** → ART-level functionality
- **Story** → Team-level implementation
- **Task** → Subtasks for implementation

**EPIC DESIGN PRINCIPLES:**

1. **User-Value First**: Each epic must enable users to accomplish something meaningful
2. **SAFe Alignment**: Epics should map to Strategic Themes when possible
3. **Requirements Grouping**: Group related FRs (as Features) that deliver cohesive user outcomes
4. **Incremental Delivery**: Each epic should deliver value within a PI boundary
5. **WSJF Ready**: Structure epics for WSJF prioritization later
6. **🔗 Dependency-Free Within Epic**: Stories within an epic must NOT depend on future stories

**ENABLER SUPPORT:**

Some epics may be **Enabler Epics** (technical foundation):

- Infrastructure Enablers (DevOps, security, performance)
- Architecture Enablers (frameworks, patterns, technical debt)
- Exploration Enablers (prototypes, spikes, research)

Enablers enable future Business Epics but provide no direct user value.
...
Format the epics_list as (SAFe-aligned):

```
## Epic List

### Epic 1: [Epic Title] (Business Epic)
**Type:** Business | Enabler
**Strategic Theme Alignment:** [Theme if applicable]
...
**WSJF Inputs (estimate later):**
- User/Business Value: TBD
- Time Criticality: TBD
- Risk Reduction: TBD
- Job Size: TBD

### Epic 2: [Epic Title] (Business/Enabler Epic)
**Type:** Business | Enabler
[Epic goal statement - what users can accomplish]
**Features to implement:**
