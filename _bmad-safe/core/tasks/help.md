...
1. **Load catalog** — Load `{project-root}/_bmad-safe/_config/bmad-help.csv`

2. **Resolve output locations** — Scan each folder under `_bmad-safe/` (except `_config`) for `config.yaml`. For each workflow row, resolve its `output-location` variables against that module's config so artifact paths can be searched.
